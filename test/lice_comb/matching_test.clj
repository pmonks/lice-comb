...
(deftest unidentified?-tests
  ...
  (testing "Unidentified LicensRef"
    (is (true?  (unidentified? (lcis/name->unidentified-license-ref))))
    (is (true?  (unidentified? (lcis/name->unidentified-license-ref "foo"))))
...
(deftest name->expressions-tests
  ...
  (testing "Cursed LGPL variations"
    ...
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "Library GPL")))
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "Lesser GNU")))
  ...
  (testing "Expressions with weird operators"
    ...
    (is (valid= #{"Apache-2.0 AND MIT"}                 (name->expressions "Apache License 2.0 with MIT licence")))
    ...
    (is (valid= #{"GPL-2.0-only WITH Classpath-exception-2.0"} (name->expressions "GPL 2.0 and classpath exception")))
    (is (valid= #{"GPL-2.0-only WITH Classpath-exception-2.0"} (name->expressions "GPL 2.0 or classpath exception")))
    (is (valid= #{"GPL-2.0-only WITH Classpath-exception-2.0"} (name->expressions "GPL 2.0 and/or classpath exception")))
...
;    ))
  ...
  (testing "Names seen in handpicked POMs on Maven Central"
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software License")))  ; Missing version - we assume the latest
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software License, Version 2.0")))
    ...
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Public License (EPL)")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Public License")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Public License")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser General Public License (LGPL)")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser General Public License")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Library General Public License")))  ; Missing version - we assume 2.0-or-later
  ...
  (testing "All names seen in POMs on Clojars as of 2023-07-13"
    ...
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "Affero General Public License")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "Affero General Public License,")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "AGPL")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "GNU Affero General Public License")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "GNU Affero General Public Licence")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"AGPL-1.0-or-later"}                  (name->expressions "GNU Affero General Public License (AGPL)")))  ; Missing version - we assume 1.0-or-later
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "APACHE LICENSE, VERSION 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "APACHE")))  ; Missing version - we assume the latest
    (is (valid= #{"Apache-2.0"}                         (name->expressions "ASL 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "ASL")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Licence")))  ; Missing clause info
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License v2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License version 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License")))  ; Missing clause info
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License, 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License, Version 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache License, Version 2.0.")))
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Public License")))  ; Missing clause info
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software License 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software License")))  ; Missing clause info
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software License, Version 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache Software Licesne")))  ; Missing clause info
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache")))  ; Missing clause info
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache, Version 2.0")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache-2.0 License")))
    (is (valid= #{"Apache-2.0"}                         (name->expressions "Apache-2.0")))
    ...
    (is (valid= #{"Apache-2.0"}                         (name->expressions "apache")))  ; Missing version - we assume the latest
    (is (valid= #{"Apache-2.0"}                         (name->expressions "apache-2.0")))
    (is (valid= #{"Artistic-2.0" "GPL-1.0-or-later"}    (name->expressions "Artistic License/GPL")))  ; Missing conjunction, so return 2 (singleton) expressions
    (is (valid= #{"Artistic-2.0"}                       (name->expressions "Artistic License")))  ; Missing version - we assume the latest
...
;    (is (valid= #{"BSD-3-Clause"}                       (name->expressions "https://github.com/mixradio/multi-atom/blob/master/LICENSE")))                       ; NO LONGER EXISTS
    ...
    (is (valid= #{"CC-BY-4.0"}                          (name->expressions "Creative Commons Attribution License")))  ; Missing version - we assume the latest
    (is (valid= #{"CC-BY-NC-3.0"}                       (name->expressions "Creative Commons Attribution-NonCommercial 3.0")))
    (is (valid= #{"CC-BY-NC-4.0"}                       (name->expressions "CC BY-NC")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"EPL-2.0" "MIT"}                      (name->expressions "Eclipse Public MIT")))  ; Missing version - we assume the latest  ; Missing conjunction, so return 2 (singleton) expressions
    ...
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Distributed under the Eclipse Public License, the same as Clojure.")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "ECLIPSE PUBLIC LICENSE")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "EPL")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "EPL-2.0")))
    (is (valid= #{"EPL-2.0"}                            (name->expressions "EPLv2")))
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse License")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Public Licence")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Public License (EPL)")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse Pulic License")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse public license, the same as Clojure")))
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Eclipse")))  ; Missing version - we assume the latest
    (is (valid= #{"EPL-2.0"}                            (name->expressions "Some Eclipse Public License")))  ; Missing version - we assume the latest
    ...
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "GNU General Public License (GPL)")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "GNU General Public License")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "GNU Public License")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "GNU")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "GPL")))  ; Missing version - we assume 1.0-or-later
    (is (valid= #{"GPL-1.0-or-later"}                   (name->expressions "The GNU General Public License")))  ; Missing version - we assume 1.0-or-later
    ...
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU LESSER GENERAL PUBLIC LICENSE")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser GPL")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser General Public Licence")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser General Public License (LGPL)")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser General Public License")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser Genereal Public License")))  ; Note spelling, listed license missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Lesser Public License")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "GNU Library or Lesser General Public License (LGPL)")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "Gnu Lesser Public License")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "LGPL License")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "LGPL Open Source license")))  ; Missing version - we assume 2.0-or-later
    (is (valid= #{"LGPL-2.0-or-later"}                  (name->expressions "LGPL")))  ; Missing version - we assume 2.0-or-later
    ...
    (is (valid= #{"MPL-2.0"}                            (name->expressions "MPL")))  ; Missing version - we assume the latest
    (is (valid= #{"MPL-2.0"}                            (name->expressions "MPL-2.0")))
    ...
    (is (valid= #{"SGI-B-2.0"}                          (name->expressions "SGI")))  ; Missing version - we assume the latest
...
(deftest name->expressions-info-tests
  ...
  (testing "Single expressions"
    (is (valid-info= {"GPL-2.0-only WITH Classpath-exception-2.0" (list {:type :concluded :confidence :low :strategy :expression-inference :source (list "GNU General Public License, version 2 with the GNU Classpath Exception")}
                                                                    {:id "GPL-2.0-only"            :type :concluded :confidence :medium :confidence-explanations #{:partial-version :missing-version-suffix} :strategy :regex-matching :source (list "GNU General Public License, version 2 with the GNU Classpath Exception" "GNU General Public License, version 2")}
    ...
    (is (valid-info= {"GPL-2.0-only WITH Classpath-exception-2.0" (list {:type :concluded :confidence :high :strategy :expression-inference :source (list "GNU General Public License, version 2.0 with the Classpath Exception 2.0")}
                                                                    {:id "GPL-2.0-only"            :type :concluded :confidence :high :confidence-explanations #{:missing-version-suffix} :strategy :regex-matching :source (list "GNU General Public License, version 2.0 with the Classpath Exception 2.0" "GNU General Public License, version 2.0")}
